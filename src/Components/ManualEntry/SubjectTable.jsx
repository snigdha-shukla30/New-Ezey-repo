...
// Table below QuickAddSubject to show manually added subjects
export default function SubjectTable({ data = [] }) {
  ...
  // Recalculate overflow when data changes
  useEffect(() => {
    const el = scrollRef.current;
    if (!el) return;
  ...
  return (
    ...
    >
      {/* Header row with blue underline */}
      ...
      >
        <div className="px-8">
          ...
          {data.length === 0 && (
            <div className="py-6 text-sm text-gray-500">No subjects added.</div>
          )}
        </div>
      </div>

      {/* Static scrollbar layout on the right side */}
      {hasOverflow && (
