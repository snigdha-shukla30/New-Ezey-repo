...
export default function GenerateTT() {
  ...
  // =====================================================
  // ✅ Generate timetable (navigate to preview screen)
  // =====================================================
  const handleGenerate = async () => {
    ...
    try {
      ...
      // ✅ HTML VISUAL CALL (your working API)
      const html = await getTimetableVisualHTML(selectedBatchId);

      if (html) {
        // ✅ navigate to preview route
        navigate(`/timetable/preview/${selectedBatchId}`);
      } else {
        alert("No timetable HTML received");
      }
    } catch (err) {
      console.error(err);
      alert(err.message || "Failed to load timetable");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="w-full h-full min-h-[calc(95vh-180px)] flex flex-col">
      ...
      <div className="flex gap-8 border-b border-[#E5E7EB] mb-6 relative">
        ...
        <button
          ...
          className={`relative px-4 pb-3 text-[14px] font-semibold leading-[150%] text-center font-mulish
